...
def get_stream_url(lecture_id: int, db: Session = Depends(get_db), current_user: User = Depends(oauth2.check_role(["student", "instructor", "admin"]))):
    ...
    if not enrollment and current_user.role not in ["instructor", "admin"]:
...
def update_progress(
    ...
    current_user: User = Depends(oauth2.check_role(["student", "instructor", "admin"]))
):
    lecture = db.query(Lecture).filter(Lecture.id == lecture_id).first()
    if not lecture:
        raise HTTPException(status_code=404, detail="Lecture not found")
...
def get_notes(lecture_id: int, db: Session = Depends(get_db), current_user: User = Depends(oauth2.check_role(["student", "instructor", "admin"]))):
